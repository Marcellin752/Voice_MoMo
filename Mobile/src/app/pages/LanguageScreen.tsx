...
import { fetchLanguageFromApi, getLanguage, updateLanguageOnApi } from "../utils/localData";
...
export default function LanguageScreen() {
  ...
  useEffect(() => {
    fetchLanguageFromApi()
      ...
      .catch(() => {
        // Keep local language in offline mode.
      });
  }, []);
  ...
  const onSave = async () => {
    ...
    setSaved(false);
    try {
      await updateLanguageOnApi(language);
      setSaved(true);
    } catch {
      setSaved(false);
    } finally {
  ...
  return (
    <div className="min-h-full px-6 py-8 bg-slate-50 dark:bg-[#121212]">
      ...
      <div className="space-y-3 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1A1A1A] p-5">
        ...
        <button type="button" onClick={onSave} disabled={loading} className="mt-2 w-full rounded-2xl bg-[#FFCC00] px-4 py-3 font-bold text-[#004F71] disabled:opacity-60">
