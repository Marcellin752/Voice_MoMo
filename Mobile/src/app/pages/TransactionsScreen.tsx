...
import { fetchTransactionsFromApi, getTransactions } from "../utils/localData";
...
export default function TransactionsScreen() {
  ...
  const [data, setData] = useState<Transaction[]>(() => getTransactions());
  ...
  const loadTransactions = () => {
    fetchTransactionsFromApi()
      ...
      .catch(() => {
        ...
        setMessage("Mode hors ligne: historique local charge.");
      });
  };
  ...
  return (
    <div className="flex flex-col min-h-full w-full bg-slate-50 dark:bg-[#121212] px-6 py-8 transition-colors duration-300">
      ...
      {showInfo && (
        <div className="mb-5 rounded-2xl bg-white dark:bg-[#1A1A1A] border border-slate-200 dark:border-white/10 px-4 py-3 text-sm text-slate-600 dark:text-zinc-300">
          Utilisez la recherche et les filtres pour retrouver une opération précise.
        </div>
      )}
      {message && (
        <div className="mb-5 rounded-2xl bg-blue-50 dark:bg-[#004F71]/20 border border-blue-100 dark:border-[#004F71]/40 px-4 py-3 text-sm text-[#004F71] dark:text-[#FFCC00]">
          {message}
        </div>
      )}
      ...
      <div className="flex space-x-3 mb-8">
        ...
        <button
          ...
          className="bg-white dark:bg-[#1A1A1A] border border-slate-200 dark:border-white/5 p-4 rounded-2xl text-slate-500 hover:text-[#004F71] dark:hover:text-[#FFCC00] transition-colors shadow-sm"
          aria-label="Basculer l'ordre des transactions"
        >
          <Filter size={20} />
        </button>
      </div>
      ...
      <div className="flex space-x-3 mb-8 overflow-x-auto no-scrollbar pb-2">
        ...
        <FilterPill label="REÇU" icon={<ArrowDownLeft size={16} />} active={filter === "in"} onClick={() => setFilter("in")} />
      ...
      {filteredTransactions.length === 0 ? (
        ...
        <div className="space-y-8">
          {Object.entries(groups).map(([dayLabel, transactions]) => (
            <div key={dayLabel}>
              <h3 className="text-xs font-bold tracking-widest text-slate-500 uppercase mb-4 px-2">{dayLabel}</h3>
              <div className="space-y-3">
                {transactions.map((tx) => (
                  <TransactionItem
                    ...
                    time={tx.time}
                    amount={tx.amount}
                    icon={tx.type === "out" ? <ArrowUpRight size={20} /> : <ArrowDownLeft size={20} />}
                  />
                ))}
              </div>
            </div>
