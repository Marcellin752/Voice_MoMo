...
import { fetchSecurityFromApi, getPinUpdatedAt, updatePinOnApi } from "../utils/localData";
...
export default function PinScreen() {
  ...
  useEffect(() => {
    fetchSecurityFromApi()
      .then((payload) => setLastUpdated(payload.pinUpdatedAt))
      .catch(() => setLastUpdated(getPinUpdatedAt()));
  }, []);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const payload = await updatePinOnApi({ oldPin, newPin, confirmPin });
      setLastUpdated(payload.pinUpdatedAt);
      setOldPin("");
      setNewPin("");
      setConfirmPin("");
      setMessage("PIN mis a jour avec succes.");
    } catch (error) {
      const text = error instanceof Error ? error.message : "Impossible de mettre a jour le PIN.";
      setMessage(text);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-full px-6 py-8 bg-slate-50 dark:bg-[#121212]">
      ...
      <form onSubmit={onSubmit} className="space-y-4 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1A1A1A] p-5">
        ...
        <button type="submit" disabled={loading} className="w-full rounded-2xl bg-[#FFCC00] px-4 py-3 font-bold text-[#004F71] disabled:opacity-60">
          {loading ? "Chargement..." : "Modifier le PIN"}
        </button>
      </form>
